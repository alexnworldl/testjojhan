  ...
  showCupon(){
    this._cuponService.showCupon(this.cupone_id).subscribe((resp:any) => {
      ...
      if(this.type_segment == 1){
        ...
        this.cupone_selected.categorias.forEach(product_s => {
          this.categorias.forEach(product => {
            if(product._id == product_s._id){
              this.categorias_selected.push(product);
            }
          });
        });
        console.log(this.categorias_selected);
      }
